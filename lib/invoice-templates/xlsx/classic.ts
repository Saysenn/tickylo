...
// Primary fill header, alternating rows, bottom-only borders on data cells.
...
export async function buildClassicXlsx(
	...
	const btm   = (c = "D8DCE8"): Partial<ExcelJS.Borders> => ({ bottom: { style: "thin" as const, color: { argb: `FF${c}` } } });
	const hFont = (): Partial<ExcelJS.Font> => ({ bold: true, color: { argb: "FFFFFFFF" }, size: 9 });
	...
	if (type === "client") {
		...
		ws.getCell("F1").font  = { bold: true, size: 20, color: { argb: `FF${PRI}` } };
		...
		// Rows 4–7: Bill To (left) | Invoice meta (right)
		...
		ws.getCell("A5").font  = { bold: true, size: 12, color: { argb: `FF${PRI}` } };
		...
		ws.getCell("A7").font  = { size: 8, italic: true, color: { argb: "FFA0A5BC" } };
		...
		ws.getCell("A9").font  = { size: 8, italic: true, color: { argb: "FFA0A5BC" } };

		ws.getRow(10).height = 4;

		// Column headers — primary fill, white text
		...
		// Data rows — alternating fill, bottom border only
		let row = 12;
		for (const item of lineItems) {
			...
			buildRowValues(item, cols).forEach((val, i) => {
				...
				cell.border = btm();
				cell.font   = { size: 9, color: { argb: "FF2A2D47" } };
		...
		const addTot = (label: string, val: string, bold = false) => {
			...
			lc.value = label; lc.font = { size: 9, bold, color: { argb: "FF9BA0B8" } };
			...
			vc.border = btm(bold ? PRI : "D8DCE8");
			ws.getRow(row).height = bold ? 22 : 17; row++;
		};
		...
		ws.getCell(`A${row}`).font  = { italic: true, size: 8, color: { argb: "FFC0C5D8" } };
		ws.getCell(`A${row}`).alignment = { horizontal: "center" };

	} else {
		buildClassicTally(wb, data, config, PRI, DARK, btm, hFont);
	}
}
...
function buildClassicTally(
	...
	PRI: string,
	DARK: string,
	btm: (c?: string) => Partial<ExcelJS.Borders>,
	...
	ws.getCell("A3").font  = { size: 8, color: { argb: "FF9BA0B8" } };
	ws.getRow(3).height = 15; ws.getRow(4).height = 6;
	...
	for (const [clientName, items] of grouped) {
		...
		gl.alignment = { indent: 1 }; ws.getRow(row).height = 16; row++;
		...
		for (const item of items) {
			...
			vals.forEach((val, i) => {
				...
				cell.fill   = { type: "pattern", pattern: "solid", fgColor: { argb: alt ? `FF${DARK}` : "FFFFFFFF" } };
				cell.border = btm(); cell.font = { size: 9 };
		...
		ws.getCell(`I${row}`).value = currency(net, cur);
		ws.getCell(`I${row}`).font  = { bold: true, size: 9.5, color: { argb: `FF${PRI}` } };
