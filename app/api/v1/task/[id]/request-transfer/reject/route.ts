...
/** PATCH /api/v1/task/[id]/request-transfer/reject — admin rejects */
	...
	try {
		...
		await prisma.$transaction([
			...
			prisma.taskComment.create({
				data: {
					task_id: id,
					user_id: admin.id,
					body: `${adminName} rejected the transfer request${reason ? ` — "${reason}"` : ""}.`,
					is_system: true,
				},
			}),
		]);
		...
		if (request.task.user_id) {
			createNotification({
				...
				body: `Your transfer request for "${request.task.title}" was rejected${reason ? `: ${reason}` : ""}.`,
		...
		console.error("[request-transfer:reject]", err);
